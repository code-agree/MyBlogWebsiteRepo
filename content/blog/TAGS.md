...
## 六、新文章发布 Checklist（历史踩坑沉淀）

1. **标题唯一**：只写 frontmatter `title`，正文不要再写 `# H1`（主题会渲染 title，正文 H1 会出现双标题）；正文可用 `>` 引导语开头（站内惯例）。
2. **date 不晚于当前时刻**：Hugo 默认不渲染未来时间戳的文章——文章会静默缺席、且引用它的 `ref` 会构建失败（已踩两次：2026-08-24 parser 篇、2026-08-25 page_table 篇）。
3. **TAGS.md 登记**：在第四节表格中按文件名顺序加一行。
4. **合并/删除旧文时**：在承接文章 frontmatter 加 `aliases = ['/blog/<旧slug>/']` 保住旧链接；全站 grep 旧 slug 确认无残留 `ref` 引用。
5. **发布前本地构建验证**：`hugo --quiet` 零报错 + 确认产物目录里有新文章。
