...
enP11p4s0 承担了 **98.6%** 的中断负载。这意味着你的 93 条连接中，绝大多数（尤其是高频的 MD 行情连接）都绑定在这一张网卡上。
...
每个队列在 2 个 core 上有大量计数，而非集中在 1 个 core。这是 **irqbalance 动态迁移** 的特征。中断从一个 core 迁移到另一个 core 时，新 core 的 cache 是冷的，会导致：
...
1. **6 张网卡中只有 1 张承担了 98.6% 的流量**，硬件资源严重浪费，不同类型的流量（MD/TD/信号）在 NIC 硬件层面互相竞争。
