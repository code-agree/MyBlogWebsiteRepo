...
**需求**：US服务器 (172.18.0.219) 通过 HK服务器 (192.168.24.102) 代理访问 OKX WebSocket服务

**网络环境**：专线连接的私有网络，HK服务器有公网IP (47.83.183.226)
...
# 允许US服务器访问
acl us_server src 172.18.0.219/32
...
```bash
# 在US服务器上测试连通性
nc -zv 192.168.24.102 18000

# 测试HTTP代理
curl -x http://192.168.24.102:18000 http://httpbin.org/ip -v

# 测试HTTPS代理
curl -x http://192.168.24.102:18000 https://httpbin.org/ip -v
```
...
# 方法1：环境变量设置
export https_proxy=http://192.168.24.102:18000
wscat -c wss://colows-d.okx.com/ws/v5/public -x '{"id": "1512", "op": "subscribe", "args": [{"channel": "trades", "instId": "BTC-USDT"}]}'

# 方法2：直接指定代理
wscat -c wss://colows-d.okx.com/ws/v5/public \
  --proxy http://192.168.24.102:18000 \
...
```bash
# 精确的源IP控制
acl us_server src 172.18.0.219/32  # 单IP精确控制
